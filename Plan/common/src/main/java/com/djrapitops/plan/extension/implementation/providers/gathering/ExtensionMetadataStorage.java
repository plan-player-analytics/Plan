...
class ExtensionMetadataStorage {
    ...
    public synchronized void storeProvider(
        ...
        MetadataKey key = new MetadataKey(database, false, information.getPluginName(), information.getName());
    ...
    public synchronized void storeTableProvider(
        ...
        MetadataKey key = new MetadataKey(database, true, information.getPluginName(), information.getName());
        ...
        try {
            for (Icon icon : table.getIcons()) {
                if (icon != null) database.executeTransaction(new StoreIconTransaction(icon));
            }
            invalidateIfNotStored(database.executeTransaction(providerTransaction), providerTransaction, key, fingerprint);
        } catch (RuntimeException executionFailure) {
            storedMetadata.remove(key, fingerprint);
    ...
    private static final class MetadataKey {
        ...
        private MetadataKey(Database database, boolean tableProvider, String pluginName, String providerName) {
        ...
        public boolean equals(Object other) {
            ...
            return database == that.database &&
                    tableProvider == that.tableProvider &&
        ...
        @Override
        public int hashCode() {
            return Objects.hash(System.identityHashCode(database), tableProvider, pluginName, providerName);
        }
    }
