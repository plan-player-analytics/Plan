...
public abstract class SQLDB extends AbstractDatabase {
    ...
    private long initialConnectionRetryDelayMs = INITIAL_CONNECTION_RETRY_DELAY_MS;
    private long maxConnectionRetryDelayMs = MAX_CONNECTION_RETRY_DELAY_MS;
    ...
    private void executeTransactionWithConnectionRetry(Transaction transaction) {
        long retryDelayMs = initialConnectionRetryDelayMs;
        ...
        while (getState() != State.CLOSED) {
            try {
                ...
                if (!waitForConnectionRetry(retryDelayMs)) throw failure;
                retryDelayMs = Math.min(retryDelayMs * 2L, maxConnectionRetryDelayMs);
            }
        }
    }
    ...
    void setConnectionRetryDelays(long initialDelayMs, long maxDelayMs) {
        this.initialConnectionRetryDelayMs = initialDelayMs;
        this.maxConnectionRetryDelayMs = maxDelayMs;
    }

    void resetConnectionRetryDelays() {
        setConnectionRetryDelays(INITIAL_CONNECTION_RETRY_DELAY_MS, MAX_CONNECTION_RETRY_DELAY_MS);
    }
