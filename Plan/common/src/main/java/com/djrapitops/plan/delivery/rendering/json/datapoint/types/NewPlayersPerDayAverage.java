...
/**
 * Datapoint for new players average per day.
 * <p>
 * Average is computed using the timezone configured in the config,
 * splitting day at midnight and counting new players each day,
 * and then averaging the number.
 *
 * @author AuroraLS3
 */
@Singleton
public class NewPlayersPerDayAverage implements Datapoint<Integer> {

    private final PlanConfig config;
    private final DBSystem dbSystem;

    @Inject
    public NewPlayersPerDayAverage(PlanConfig config, DBSystem dbSystem) {
        this.config = config;
    ...
    public Optional<Integer> getValue(GenericFilter filter) {
        if (filter.getPlayerUUID().isPresent()) {
            throw new BadRequestException("NEW_PLAYERS_AVERAGE does not support player parameter");
        }

        if (!filter.getServerUUIDs().isEmpty()) {
            return Optional.of(dbSystem.getDatabase().query(PlayerCountQueries.averageNewPlayerCount(
    ...
    public WebPermission getPermission(GenericFilter filter) {
        if (filter.getPlayerUUID().isPresent()) {
            ...
            return WebPermission.DATA_SERVER_NEW_PLAYERS_AVERAGE;
        } else {
            return WebPermission.DATA_NETWORK_NEW_PLAYERS_AVERAGE;
        }
    }

    @Override
    public DatapointType getType() {
        return DatapointType.NEW_PLAYERS_AVERAGE;
    }
}
