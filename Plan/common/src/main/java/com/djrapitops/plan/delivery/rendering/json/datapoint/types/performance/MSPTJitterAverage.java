...
/**
 * Datapoint for looking up Average MSPT Jitter within the timeframe.
 *
 * @author AuroraLS3
 */
@Singleton
public class MSPTJitterAverage implements Datapoint<Double> {

    private final DBSystem dbSystem;

    @Inject
    public MSPTJitterAverage(DBSystem dbSystem) {
    ...
    public Optional<Double> getValue(GenericFilter filter) {
        double average = dbSystem.getDatabase().query(TPSQueries.averageMsptJitter(
                filter.getAfter(),
                filter.getBefore(),
    ...
    public WebPermission getPermission(GenericFilter filter) {
        if (filter.getPlayerUUID().isPresent()) {
            ...
            return WebPermission.DATA_SERVER_MSPT_JITTER_AVERAGE;
        } else {
            return WebPermission.DATA_NETWORK_MSPT_JITTER_AVERAGE;
        }
    }

    @Override
    public DatapointType getType() {
        return DatapointType.MSPT_JITTER_AVERAGE;
    }
