...
public class PlayersOnlineAverage implements Datapoint<Double> {
    ...
    public Optional<Double> getValue(GenericFilter filter) {
        if (filter.getPlayerUUID().isPresent()) {
            throw new BadRequestException("PLAYERS_ONLINE_AVERAGE does not support player parameter");
        }

        if (filter.getServerUUIDs().isEmpty()) {
            throw new BadRequestException("PLAYERS_ONLINE_AVERAGE is only available for servers");
        }
