...
public interface TopListQueriesTest extends DatabaseTestPreparer {
    ...
    default void topPlaytimeListQueryReturnsSinglePlayer() {
        ...
        String expected = TestConstants.PLAYER_ONE_NAME;
        String result = db().query(TopListQueries.fetchNthTop10PlaytimePlayerOn(serverUUID(), 0, 0, System.currentTimeMillis()))
                .orElseThrow(AssertionError::new)
                .getPlayerName();
        assertEquals(expected, result);
    }
    ...
    @Test
    default void topKillCountListQueryReturnsSinglePlayer() {
        storeSessionForTopListQueries();

        String expected = TestConstants.PLAYER_ONE_NAME;
        String result = db().query(TopListQueries.fetchNthTop10PlayerKillCountOn(serverUUID(), 0, 0, System.currentTimeMillis()))
                .orElseThrow(AssertionError::new)
                .getPlayerName();
        assertEquals(expected, result);
    }
