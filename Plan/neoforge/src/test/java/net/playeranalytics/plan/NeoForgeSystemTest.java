...
import utilities.mocks.NeoForgeMockComponent;
...
/**
 * Test for NeoForge PlanSystem.
 */
class NeoForgeSystemTest {
    ...
    void prepareSystem(@TempDir Path temp) {
        system = new NeoForgeMockComponent(temp).getPlanSystem();
        system.getConfigSystem().getConfig()
                .set(WebserverSettings.PORT, TEST_PORT_NUMBER);
    }

    @Test
    void neoForgeSystemEnables() {
        try {
            system.enable();
    ...
    @Test
    void neoForgeSystemHasDefaultConfigValuesAfterEnable() throws IllegalAccessException {
        try {
            system.enable();
